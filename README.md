...
授权清单见 [docs/AUTHORIZATION_CHECKLIST.md](./docs/AUTHORIZATION_CHECKLIST.md)。当前 Worker API + D1 已按无绑卡方案部署；后续授权重点是替换 Apple Team ID / App Store ID，并完成 iPhone 真机安装。
...
- `STORAGE_PROVIDER=local` 当前无绑卡方案；需要长期保存模型文件时再切到 `r2` 或其他对象存储
- `R2_ACCOUNT_ID`、`R2_ACCESS_KEY_ID`、`R2_SECRET_ACCESS_KEY`、`R2_BUCKET`，仅启用 R2 时需要
- `DATABASE_URL`
